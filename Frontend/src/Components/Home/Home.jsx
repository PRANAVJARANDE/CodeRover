...
function Home() {
    ...
    return (
        <main className="min-h-screen bg-slate-950 px-5 py-8 text-white lg:px-8">
            <section className="mx-auto grid min-h-[calc(100vh-8rem)] max-w-6xl items-center gap-7 lg:grid-cols-[0.82fr_1.18fr]">
                ...
                <div className="rounded-3xl border border-white/10 bg-slate-900/70 p-6 shadow-2xl shadow-black/20 lg:p-8">
                    ...
                    <div className="mt-6 grid gap-4">
                        {actionCards.map((card) => (
                            <Link
                                key={card.title}
                                to={card.to}
                                className="group rounded-2xl border border-white/10 bg-slate-800/80 p-5 transition hover:-translate-y-1 hover:border-cyan-300/40 hover:bg-slate-800"
                            >
